...
#----------------------------------------------
$Install_Office = 0 #installs Office 365 using config values set in this script
$Install_Office_Edition = 'O365BusinessRetail' #Office edition to be installed
$Install_Office_Edition = $Install_Office_Edition.split(',') | Select -First 1
$Install_Office_Download_Path = 'C:\Office365Install' #directory where the Office 365 setup and ODT files should be downloaded
...
$Install_Office_WITH_ODT = 0 #installs Office using an existing Office 365 ODT configuration XML file
$Install_Office_WITH_ODT_XMLFILE = 'C:\Office365Install\365ODT-Config.xml' #Full path to an existing Office365 ODT Configuration XML file to use with deployment.To config or create a configuration file, recommend visiting https://config.office.com

$Remove_Office_Only = 0 #removes any detected office installations without installing another
$Remove_Office_Only_Edition = 'All' #optionally removes only this edition of office

#The following variables are ignored if $Install_Office_WITH_ODT_XMLFILE is configured
$Install_Office_Channel = 'Current' #Defines which channel to use for installing Office
$Install_Office_Channel = $Install_Office_Channel.split(',') | Select -First 1
$Install_Office_Source_Path = 'Internet' #Path where Office installation files will be saved and deployed from.
$Install_Office_Exclude_Apps = 'Groove,Lync' #Specifies the applications that should be excluded from installing with Office, comma separated
$Install_Office_Exclude_Apps = $Install_Office_Exclude_Apps.split(',')
$Install_Office_Org_Name = 'My Organization' #Specifies your organization name for Office installs
$Install_Office_Shared_Computer_Licensing = 0 #Enables configuring office to use Shared Computer Licensing during Office installs
$Install_Office_Remove_Previous_Intalls = 1 #Enables removal of any previous non-365 (MSI installed) versions of Office found when installing Office 365
$Install_Office_Accept_EULA = 1 #Enables automatically accepting EULA for Office 365 installs
$Install_Office_Enable_Updates = 1 #Enables automatic updates for Office instal
$Install_Office_Display_Install = 0 #Enables displaying UI while Installing Office. Leave disabled for silent installs
$Install_Office_AUTO_ACTIVATE = 1 #Enables automatic activation of Office Installs
$Install_Office_FORCE_APPSHUTDOWN = 1 #Enables force closing any open Office apps or processes when installing Offic
#---------------------------------------------- 
...
$Required_modules = '' #these modules are automatically installed and imported if not already
$enablelogs = 1 #Enables output of debug messages, errors and others to a log file
$logfile_directory = 'C:\Logs' #Directory where log file should be created
$Copy_SetupLog = 1 #Copies Office 365 setup logs to the log directory specified in "LogFile_Directory"
$LogFileAppend = $true #enables appending to existing log file. Disabled overwrites previous log file content
$logtime = $true #Displays the date/time for each log entry in the log file
...
function Request-Config 
{
  ...
  param (
    ...
    [string]$Install_Office_Org_Name = 'My Organization',
    [validateset('True','False')]
...
if($Local_Test_Run){
  ...
  {
    Write-Verbose -Message "[ERROR] An exception occured setting configuration values:`n | $($_.exception.message)`n | $($_.InvocationInfo.positionmessage)`n | $($_.ScriptStackTrace)`n"
    exit
  }
